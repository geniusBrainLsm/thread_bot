...
public class MultiSiteNewsCrawlerService implements NewsCrawlerService {
    ...
    public Optional<NewsArticle> crawlLatestAiNews() {
        ...
        if (activeSources.isEmpty()) {
            log.warn("No active news sources configured");
        ...
        for (NewsSource source : activeSources) {
            try {
                ...
                if (article.isPresent()) {
                    log.info("Successfully crawled article from source: {}", source.getName());
                    return article;
                }
            } catch (Exception e) {
                log.warn("Failed to crawl from source: {}, trying next source", source.getName(), e);
            }
        }
        
        log.warn("No articles found from any configured source");
        return Optional.empty();
    }
    
    public List<NewsArticle> crawlAllSources() {
        List<NewsSource> activeSources = newsSourceRepository.findByIsActiveTrue();
    ...
    private Optional<NewsArticle> crawlFromSource(NewsSource source) {
        try {
            log.info("Crawling from source: {} at {}", source.getName(), source.getBaseUrl());
            ...
            log.info("Successfully crawled article: {} from {}", title, source.getName());
